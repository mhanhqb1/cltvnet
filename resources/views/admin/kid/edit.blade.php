...
@section('content')
<form action="{{ route('admin.kid.save') }}" method="POST" enctype="multipart/form-data">
    @csrf
        ...
        <div class="col-md-12">
            <div class="card card-secondary">
                <div class="card-header">
                    <h3 class="card-title">
                        Danh sách video
                        <a href="{{ route('admin.kid.addVideo', ['movie_id' => $item->id]) }}" class="btn btn-primary">Add video</a>
                    </h3>
                    <div class="card-tools">
                        <button type="button" class="btn btn-tool" data-card-widget="collapse" title="Collapse">
                            <i class="fas fa-minus"></i>
    ...
    <div class="row">
        <div class="col-12">
            <a href="{{ route('admin.kid.index') }}" class="btn btn-secondary">Hủy bỏ</a>
...
<script>
    $(document).ready(function() {
        ...
        $('#videoDataTable').DataTable({
            processing: true,
            serverSide: true,
            ajax: '{!! route('admin.kid.indexDataVideo', ['movie_id' => $item->id]) !!}',
            columns: [
                { data: 'id', name: 'id' },
