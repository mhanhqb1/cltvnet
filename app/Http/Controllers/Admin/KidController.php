...
class KidController extends Controller
{
    ...
    public function indexData()
    {
        ...
        return Datatables::of($data)
            ...
            ->addColumn('action', function ($item) {
                return '<a href="'.route('admin.kid.edit', $item->id).'" class="btn btn-xs btn-primary"><i class="fas fa-edit"></i> Edit</a>
                <a href="'.route('admin.kid.addVideo', ['movie_id' => $item->id]).'" class="btn btn-xs btn-info"><i class="fas fa-edit"></i> Add video</a>
                <form action="'.route('admin.kid.delete', $item->id).'" method="POST" style="display:inline-block;">
    ...
    public function add()
    {
        ...
        return view('admin.kid.add', compact('cates', 'countries'));
    }
    ...
    public function edit($id)
    {
        ...
        return view('admin.kid.edit', compact('item', 'cates', 'movieCates', 'countries'));
    }
    ...
    public function save(Request $request)
    {
        ...
        if (!empty($request->image)) {
            $image = $request->file('image')->storePubliclyAs($this->dirAlias, $slug.'-'.time().'.jpg', 'public');
        ...
        $item->ok_ru_id = !empty($request->ok_ru_id) ? $request->ok_ru_id : null;
        if (!empty($image)) {
            $item->image = $image;
        }
        if ($item->save()) {
            ...
            return redirect()->route('admin.kid.index')->with('success', 'Dữ liệu đã được cập nhật thành công');
        }
        return redirect()->route('admin.kid.index')->with('error', 'Dữ liệu cập nhật bị lỗi');
    }
    ...
    public function delete($id)
    {
        ...
        return redirect()->route('admin.kid.index')->with('success', 'Dữ liệu đã được xóa thành công');
    }

    public function addVideo()
    {
        $movies = Movie::where('cate_type', $this->cateType)->get();
        return view('admin.kid.add_video', compact('movies'));
    }
    ...
    public function editVideo($id)
    {
        ...
        $movies = Movie::where('cate_type', $this->cateType)->get();
        return view('admin.kid.edit_video', compact('item', 'movies'));
    }
    ...
    public function saveVideo(Request $request)
    {
        ...
        if ($item->save()) {
            return redirect()->route('admin.kid.edit', $item->movie_id)->with('success', 'Dữ liệu đã được cập nhật thành công');
        }
        return redirect()->route('admin.kid.edit', $item->movie_id)->with('error', 'Dữ liệu cập nhật bị lỗi');
    }
    ...
    public function indexDataVideo(Request $request)
    {
        ...
        return Datatables::of($data)
            ...
            ->addColumn('action', function ($item) {
                return '<a href="'.route('admin.kid.editVideo', $item->id).'" class="btn btn-xs btn-primary"><i class="fas fa-edit"></i> Edit</a>
                <form action="'.route('admin.kid.deleteVideo', $item->id).'" method="POST" style="display:inline-block;">
                    <input type="hidden" name="_method" value="delete"/>
                    '.csrf_field().'
                    <input type="submit" class="btn btn-xs btn-danger" onclick="return window.confirm(\'Bạn muốn xóa item này không?\')" value="Delete"/>
                </form>
    ...
    public function deleteVideo($id)
    {
        ...
        $video->delete();
        return redirect()->route('admin.kid.edit', $video->movie_id)->with('success', 'Dữ liệu đã được xóa thành công');
    }
}
